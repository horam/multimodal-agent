export function getChatHtml(): string {
  return `
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8" />
<style>
  body {
    font-family: sans-serif;
    padding: 10px;
  }
  #messages {
    height: 70vh;
    overflow-y: auto;
    border: 1px solid #ddd;
    padding: 8px;
    margin-bottom: 8px;
  }
  .user {
    font-weight: bold;
    margin-bottom: 4px;
  }
  .agent {
    color: #007acc;
    margin-bottom: 6px;
  }
  #input {
    width: 85%;
  }
</style>
</head>
<body>

<h3>Multimodal Agent</h3>

<div id="messages"></div>

<input id="input" placeholder="Ask the agent…" />
<button id="send">▶</button>

<script>
  const vscode = acquireVsCodeApi();
  const messages = document.getElementById("messages");
  const input = document.getElementById("input");
  const sendBtn = document.getElementById("send");

  let thinkingNode = null;

  function addMessage(role, text) {
    const div = document.createElement("div");
    div.className = role.toLowerCase();
    div.textContent = role + ": " + text;
    messages.appendChild(div);
    messages.scrollTop = messages.scrollHeight;
  }

  function showThinking() {
    thinkingNode = document.createElement("div");
    thinkingNode.className = "agent";
    thinkingNode.textContent = "Agent: …";
    messages.appendChild(thinkingNode);
    messages.scrollTop = messages.scrollHeight;
    input.disabled = true;
    sendBtn.disabled = true;
  }

  function replaceThinking(text) {
    if (thinkingNode) {
      thinkingNode.textContent = "Agent: " + text;
      thinkingNode = null;
    } else {
      addMessage("Agent", text);
    }
    input.disabled = false;
    sendBtn.disabled = false;
    input.focus();
  }

  sendBtn.onclick = sendMessage;

  input.addEventListener("keydown", (e) => {
    if (e.key === "Enter") {
      sendMessage();
    }
  });

  function sendMessage() {
    if (!input.value) return;

    addMessage("You", input.value);
    vscode.postMessage({ type: "send", text: input.value });
    input.value = "";
  }

  window.addEventListener("message", (event) => {
    const msg = event.data;

    if (msg.type === "thinking") {
      showThinking();
    }

    if (msg.type === "response") {
      replaceThinking(msg.text);
    }

    if (msg.type === "error") {
      replaceThinking(msg.text || "Something went wrong.");
    }
  });
</script>

</body>
</html>
`;
}